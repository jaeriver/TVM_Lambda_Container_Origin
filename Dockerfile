...
# git clone
RUN git clone https://github.com/jaeriver/TVM_Lambda_Container_Origin.git
...
ENV TVM_HOME=/var/task/TVM_Lambda_Container_Origin/tvm
ENV PATH=$PATH:$TVM_HOME/bin
ENV PYTHONPATH=$TVM_HOME/python:${PYTHONPATH}
ENV PATH=$TVM_HOME/python:$PATH

RUN pip3 install -r /var/task/TVM_Lambda_Container_Origin/requirements.txt

# install packages
WORKDIR TVM_Lambda_Container_Origin
RUN mkdir tvm/build
RUN cp /var/task/TVM_Lambda_Container_Origin/config.cmake tvm/build
...
RUN cp /var/task/TVM_Lambda_Container_Origin/lambda_function.py ${LAMBDA_TASK_ROOT}
